...
for ii=1:nx
    for jj=1:ny
        for kk=1:nz
            ...
            Tmat(ii,jj,kk)=T0+nu*(P/(2*pi*k*r))*exp(-v/(2*alpha)*(x(ii)+r));
...
pcolor(Y,Z,transpose(squeeze(Tmat(1,:,:))),EdgeColor="none")
title('Tlim(y,z)')
xlim([ymin ymax]);
ylim([zmin zmax]);
xlabel('y[\mum]')
...
[X,Y]=meshgrid(x*10^6,y*10^6);
[M,C]=contour(X,Y,transpose(Tmat(:,:,1)),[300 300],'ShowText','on');
M(:,1)=[];
xcords=M(1,:);
ycords=M(2,:);
width0=min(ycords);
